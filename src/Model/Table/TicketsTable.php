...
class TicketsTable extends Table
{
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ->scalar('ticket_title')
            ->maxLength('ticket_title', 255)
            ->requirePresence('ticket_title', 'create')
            ->notEmptyString('ticket_title');

        $validator
            ->scalar('ticket_message')
            ->allowEmptyString('ticket_message');

        $validator
            ->scalar('ticket_status')
            ->maxLength('ticket_status', 255)
            ->requirePresence('ticket_status', 'create')
            ->notEmptyString('ticket_status');
